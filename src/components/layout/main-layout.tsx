  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside className={cn(
          ...
          {!isCollapsed ? (
            <>
              <img 
                ...
                className="h-10 w-auto object-contain relative z-10 logo-premium mt-1" 
              />
              {/* Accent line + tagline */}
              <div className="mt-1 flex items-center gap-1.5 relative z-10">
                <div className="h-[1px] w-3.5 bg-[#00C9E0] rounded-full" />
        ...
        <nav className="flex-1 px-2.5 py-2 space-y-2 overflow-y-auto no-scrollbar flex flex-col justify-evenly">
          <div>
            {!isCollapsed && (
              <p className="px-3 mb-1 text-[9px] font-bold text-slate-400/90 tracking-[0.18em] uppercase">
                Principal
              </p>
            )}
                ...
                return (
                  <div key={item.href} className="relative group/navitem">
                    <Link
                      ...
                      className={cn(
                        "flex items-center gap-2.5 py-1.5 rounded-xl text-xs font-medium transition-all duration-300 ease-out relative overflow-hidden animate-sidebar-item",
                    ...
                    >
                      ...
                      <item.icon className={cn(
                         "h-[16px] w-[16px] transition-all duration-300 flex-shrink-0 relative z-10",
                         item.iconClass,
                         isActive 
                      ...
                      {!isCollapsed && (
                        <span className="text-[12px] relative z-10 transition-colors duration-300 truncate">{item.name}</span>
                      )}
                    </Link>
        ...
        {/* Logout Footer */}
        <div className="border-t border-sidebar-border/10 px-3 py-2 shrink-0 animate-sidebar-item relative group/logout" style={{ animationDelay: `${(mainNavItems.length + moreNavItems.length) * 50}ms` }}>
          <button
            onClick={handleLogout}
            className={cn(
              "w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold text-red-500 border border-red-500/30 bg-red-950/10 hover:bg-red-500/15 hover:text-red-400 transition-all duration-300 ease-out group",
