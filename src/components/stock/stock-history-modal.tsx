        ...
        <div className="sidebar-premium-bg px-6 py-4 relative shrink-0">
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-300/70 mt-1">{item.nombre}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-[#F0F5FA] px-5 py-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-10">
              ...
              <p className="text-[11px] font-semibold text-slate-400 mt-2">Cargando historial...</p>
            </div>
          ) : movements.length > 0 ? (
                ...
                <div key={move.id} className="relative pl-8 pb-3 border-l-2 border-slate-200 last:border-0 last:pb-0">
                  ...
                  <div className="bg-white rounded-xl border border-slate-200/60 p-3 shadow-sm">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[8px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-lg">
{new Date(move.created_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })} {new Date(move.created_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                    ...
                    {move.trabajo_id ? (
                      <Link 
                        ...
                        className="text-[10px] text-[#0097A7] font-bold hover:underline flex items-center gap-1 group"
                        onClick={onClose}
                      >
                        {move.motivo}
                      ...
                      <p className="text-[10px] text-slate-600 font-medium">{move.motivo}</p>
                    )}
                    <div className="flex items-center gap-1.5 mt-1.5">
                       <span className="text-[8px] font-bold text-slate-400 uppercase">Balance:</span>
                       <span className="text-[9px] font-black text-[#0097A7] bg-[#E6F9FB] px-1.5 py-0.5 rounded-md">{move.cantidad_resultante}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-10">
              ...
              <p className="text-[11px] font-semibold text-slate-500">No hay movimientos</p>
              <p className="text-[9px] text-slate-400 mt-0.5">para este producto</p>
            </div>
          )}
        </div>
